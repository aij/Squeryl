...
//      queryToIterable(view2QueryAll(t))
  ...
  // Allow directly comparing scalaz tagged types
  implicit def scalazTaggedComparisonEvidence[A, B, T](
    implicit underlyingCompare: CanCompare[T, T]
  ) = new CanCompare[TScalazTagged[A, B, T], TScalazTagged[A, B, T]]
